...
```
名称        URL                       操作
my_sub      https://...               [获取节点] [删除]
  ↓ 点【获取节点】后此行下方展开
  ┌─ 节点列表（来自 cache 或刚拉的） ──────┐
  │ 节点名      协议    服务器:端口          │
  │ HK_01      vmess   1.2.3.4:443       │
  │ HK_02      vmess   5.6.7.8:443       │
  │ US_01      ss      ip:port           │
  └────────────────────────────────────┘
my_sub2     https://...               [获取节点] [删除]
[+ 添加订阅]
```

- **名称**：只允许字母 / 数字 / 下划线
- **【获取节点】按钮**：调后端脚本拉这一条订阅、解析、写 cache、原地展开
- **节点展开**：点【获取节点】才展开；再点一次刷新；第三次（或点别处？）折叠 → 简化：每次点都拉 + 展开；展开后右上角加【×】折叠
...
无变化（v1 保留）。手动节点也参与全局节点池，加入 group cache 的下拉。
...
| 触发点 | 行为 |
|--------|------|
| 用户点订阅行的【获取节点】 | 拉这一条订阅、写入 cache 对应 key、展开行内列表 |
| 用户保存配置后 | 后台静默拉**所有**订阅，更新 cache，不阻塞保存，失败不报错 |
| 进入页面 | 不主动拉，直接读现有 cache 渲染 |
...
| 触发 | 行为 |
|------|------|
| 第一次打开（无配置） | 渲染：1 个空 `proxy` 组（filter 空、policy 自动最快）+ 默认 routing 3 条 + DNS 默认模板 |
